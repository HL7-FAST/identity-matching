package org.hl7.fast.operations;
...
import org.hl7.fast.common.FhirContextProvider;
import org.hl7.fast.operations.models.IdentifierQueryParams;
import org.hl7.fast.operations.models.IdentifierRegistry;
import org.hl7.fast.operations.models.IdentityMatchParams;
import org.hl7.fast.operations.models.IdentityMatchValidationLevel;
import org.hl7.fast.operations.models.IdentityMatchingScorer;
...
public class IdentityMatching {
	...
	public IdentityMatching(AppProperties appProperties, IdentityMatchingProperties identityMatchingProperties,
			IFhirResourceDao<Patient> patientDao, ResourceLoader resourceLoader) {
	...
	private void assertPatientIsValid(Patient patient, RequestDetails theRequestDetails) {
		...
		IdentityMatchValidationLevel validationLevel = identityMatchingProperties.getMatchValidationLevel();

		// override validationLevel to the value of the header if it is a valid value of the enum
		String matchHeader = theRequestDetails.getHeader(identityMatchingProperties.getMatchValidationHeader());
		if (matchHeader != null) {
			try {
				validationLevel = IdentityMatchValidationLevel.valueOf(matchHeader.toUpperCase());
			} catch (IllegalArgumentException e) {
				throw new InvalidRequestException("Invalid value for " + identityMatchingProperties.getMatchValidationHeader() + " header: \"" + matchHeader 
					+ "\". Valid values are: " 
	...
	private Bundle getPatientMatch(Patient refPatient, RequestDetails theRequestDetails) {
		...
		String remoteServerHeader = theRequestDetails.getHeader(identityMatchingProperties.getRemoteMatchHeader());
		if (remoteServerHeader != null) {
			List<String> remoteServers = new ArrayList<String>();

			if (remoteServerHeader.isBlank()) {
				remoteServers = identityMatchingProperties.getRemoteServers();
			} else {
			...
			if (remoteServers != null && !remoteServers.isEmpty()) {
				...
				if (remoteServers.size() > identityMatchingProperties.getRemoteLimit()) {
					remoteServers = remoteServers.subList(0, identityMatchingProperties.getRemoteLimit());
				}
