...
Spring finds `org.hl7.fast` through the starter's built-in extension point: `hapi.fhir.custom-bean-packages: org.hl7.fast` in `application.yaml` (component-scanned by `StarterJpaConfig`). `Application.java` is stock upstream. Test contexts do not set `custom-bean-packages`, so tests boot a pure stock server; an integration test for the custom code must set that property and `security.fetch-cert: false` (or provide a cert).

Configuration lives in `src/main/resources/application.yaml`. The project-specific knobs are the `security:` block (`enable-authentication`, `issuer`, `cert-file`/`cert-password`, `fetch-cert`, `bypass-header`) and, under `hapi.fhir:`, `match-validation-level` (`DEFAULT` | `META_PROFILE` | `NONE`), `match-validation-header` (default `X-Match-Validation`), `remote-match-header` (default `X-Remote-Match`), `remote-servers`, and `remote-limit`. Everything else in the YAML is standard HAPI starter configuration.
